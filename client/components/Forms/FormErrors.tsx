...
export default function FormErrors({ errors, field, nested }: FormErrorsProps) {
  ...
  const renderMessage = (message: string, index: number) => (
    <div
      key={index}
      className="flex items-center text-cmdr text-red-500"
      id="FormErrors"
    >
      <BsExclamationCircle className="m-1 size-6" />
      {message}
    </div>
  );

  if (Array.isArray(rawError)) {
    const messages = getNestedErrorMessages(rawError, nested);
...
function getNestedErrorMessages(
  errorArray: any[],
  nestedKeys?: string[]
): string[] {
  if (!nestedKeys) {
