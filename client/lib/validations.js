...
export const registerSchema = z
    ...
    .refine((data) => {
    return (data.id.slice(0, 4) !== "cred" || data.confirmPassword !== undefined);
}, { message: "You must enter your password twice", path: ["confirmPassword"] });
const ObjectIdSchema = z.custom((value) => value instanceof Types.ObjectId, {
...
const itemSchemaWithoutTranslations = z.object({
    ...
    pluralForm: z
        .array(z.string().max(65, "Plural forms can be no longer than 65 characters"))
...
export const parsedItemSchema = itemSchemaWithoutTranslations.merge(parsedItemSpecificSchema);
...
const populatedTranslationsSchema = z.object({
    translations: z.custom(),
});
export const itemSchemaWithTranslations = itemSchemaWithoutTranslations.merge(translationsSchema);
export const itemSchemaWithPopulatedTranslations = itemSchemaWithoutTranslations.merge(populatedTranslationsSchema);
function createRegexWithMessage({ nameOfString = "String", letters = true, numbers = false, characters, }) {
    ...
    const regex = new RegExp(`^[${letters ? "a-zA-Z" : ""}${numbers ? "0-9" : ""}${escapedCharacters}]+$`);
    // Create the validation message
    const message = `${nameOfString} may only contain ${letters ? "letters" : ""}${letters && numbers ? ", " : ""}${numbers ? "numbers" : ""}${(letters || numbers) && characters ? " and " : ""}${characters ? `these characters: ${characters}` : ""}`.trim();
