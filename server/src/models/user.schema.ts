...
import { oAuthProviders } from "@linguardian/shared/contracts";
import { SupportedLanguage } from "@linguardian/shared/contracts";
import { SensitiveUser } from "@/schemas";
            ...
            validate: {
                validator: (value: Record<string, string>) => {
                    ...
                    return Object.keys(value).every((key) =>
                        oAuthProviders.includes(
                            key as (typeof oAuthProviders)[number]
                        )
                    );
                },
                message: (props: any) =>
            ...
            validate: {
                ...
                message: (props: { value: Record<string, any> }) => {
                    const invalidKeys = Object.keys(props.value).filter(
                        (key) =>
                            !allSupportedLanguages.includes(
                                key as SupportedLanguage
                            )
                    );
                    const invalidValues = Object.entries(props.value).filter(
                        ([_, arr]) =>
                            !Array.isArray(arr) ||
                            arr.some((num) => typeof num !== "number")
                    );
