...
import { objectIdSchema, SensitiveUser, sensitiveUserSchema } from "@/schemas";
    ...
    try {
        ...
        passedItemsAndFetchedItems.forEach(async (itemPair) => {
            if (!itemPair || !itemPair.passedItem)
                throw new Error(
                    "This item was not reviewed, please report this"
                );
        ...
        logger.error("Error updating reviewed items", {
            items,
            userId,
            language,
            error: err,
    ...
    try {
        ...
        const filteredItems = learnedItemsForLanguage.filter(
            (learnedItem: LearnedItem) =>
                !newItemIds.some(
                    (newItemId) =>
                        new Types.ObjectId(learnedItem.id) === newItemId
                )
        );
        ...
        logger.error("Error adding newly learned items", {
            items,
            userId,
            language,
            error: err,
