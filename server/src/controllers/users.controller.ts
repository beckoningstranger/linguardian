...
import { registrationDataSchema } from "@linguardian/shared/contracts";
...
export async function getCurrentUserController(
    ...
    try {
        const response = await findUser({ method: "_id", query: id });
        if (!response.success) {
...
export async function isTakenController(
    req: Request,
    res: Response,
    next: NextFunction
) {
...
export async function updateUserController(
    ...
    if (trustedId !== routeUserId)
        errorResponse(
            res,
            400,
            "Authenticated user does not match route user ID"
        );
