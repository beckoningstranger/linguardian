...
import { findPopulatedListByListNumber } from "@/repositories/list.repo";
...
export async function ListOverviewDataService(
    ...
    const [userResponse, listResponse] = await Promise.all([
        findUser({ method: "_id", query: userId }),
        findPopulatedListByListNumber(listNumber),
    ]);
...
export async function EditListPageDataService(
    ...
    const listResponse = await findPopulatedListByListNumber(listNumber);
