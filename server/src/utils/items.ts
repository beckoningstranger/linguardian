...
import { DbItem, NewItem, TranslationsAsObjectIds } from "@/schemas";
import { allLanguageFeatures, supportedLanguageCodes } from "@linguardian/shared/constants";
import { normalizeString, safeDbWrite, slugifyString } from "@/utils";
import { ItemModel } from "@/models";
import { findPopulatedItemById } from "@/repositories/item.repo";
...
export async function updateRelatedItems(
  ...
  if (!isNewItem) {
    const response = await findPopulatedItemById(item.id);
    if (!response.success) {
