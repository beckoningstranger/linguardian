import { UnitToAdd } from "@/types/types";
import { ListModel } from "@/models";
import { getListByListNumber } from "@/models/list.model";
import { List, ParseResult } from "@linguardian/shared/contracts";
        ...
        if (updateResult.matchedCount === 0) {
            for (const entry of existingUnitsToAdd) {
                results.push({
                    ...
                    status: "error",
                    message:
                        "List not found while adding existing item to list",
                });
            }
        } else {
            for (const entry of existingUnitsToAdd) {
                results.push({
                    ...
                    message:
                        "Existing item added to this list (or was already present)",
                });
            }
        }
    }
